...
class TestAppBuilder(unittest.TestCase):
    def setUp(self):
        self.bd = dbmanager.BDManager("ejudge_test", "postgres", "0000", port="5433")
    ...
    def test_get_answer(self):
        ...
        self.assertEqual(result.program_text, "import sys\nprint(sys.argv[1:])")
    ...
    def test_add_status_ok(self):
        ...
        result = self.bd._make_request("SELECT id, result, error_test_id, extended_information FROM status WHERE id=%(uuid_id)s",{"uuid_id": temp_uuid})
        result = result[0]
    ...
    def test_update_solution_id(self):
        ...
        result = self.bd._make_request("SELECT status_id FROM user_solutions WHERE id=%(user_solution_id)s",{"user_solution_id": user_solution_id})
        result = result[0]
    ...
    def _test_add_status(self):
        ...
        ext = {"gcc": "ERROR!!!"}
        status_id = self.bd.add_status(user_solution_id, result, error_test_id, ext)
